...
- [x] Canvas: personal access token (keychain) + base URL; planner items with submission state, current grades, last-week announcements feed the agenda intent. Bad tokens rejected on save (verified against Rutgers). Unverified with a real token
...
- [ ] Spoken daily brief (Canvas, recruiting deadlines, calendar, overnight results)
